...
void man(char *name)
{
	...
	else if (_stricmp(name, "read") == 0)
	{
		...
		printf("Usage : read File_name No_Of_Byets_To_Read\n");
	}
	else if (_stricmp(name, "write") == 0)
	{
		printf("Description : Used to write into regular file\n");
		printf("Usage : wite File_name\n After this enter data that we waant to write\n");
	}
...
int stat_file(char *name)
{
	...
	printf("\n---------------Statistical Informtion about File-----------------\n");
	printf("File name : %s\n", temp->FileName);
	printf("Inode number : %d\n", temp->InodeNumber);
	printf("File size : %d\n", temp->FileSize);
	printf("Actual file size : %d\n", temp->FileActualSize);
...
int main()
{
	...
	while (1)
	{
		...
		if (count == 1)
		{
			...
			{
				printf("Terminating the Marvelous Virtual File System\n");
				break;
			}
			else
			{
		...
		else if (count == 3)
		{
			if (_stricmp(command[0], "create") == 0)
			{
				...
				if (ret == -1)
					printf("ERROR : incorrect parameters\n");
				if (ret == -2)
					printf("ERROR : There is no inode avaiable\n");
				if (ret == -3)
